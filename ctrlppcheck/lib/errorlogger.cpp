...
ErrorLogger::ErrorMessage::ErrorMessage(
    const std::list<FileLocation> &callStack, const std::string &file1,
    Severity::SeverityType severity, const std::string &msg,
    const std::string &id, bool inconclusive)
    : _callStack(callStack),             // locations for this error message
      _id(id),                           // set the message id
      file0(file1), _severity(severity), // severity for this error message
      _cwe(0U), _inconclusive(inconclusive) {
...
ErrorLogger::ErrorMessage::ErrorMessage(
    const std::list<FileLocation> &callStack, const std::string &file1,
    Severity::SeverityType severity, const std::string &msg,
    const std::string &id, const CWE &cwe, bool inconclusive)
    : _callStack(callStack),             // locations for this error message
      _id(id),                           // set the message id
      file0(file1), _severity(severity), // severity for this error message
      _cwe(cwe.id), _inconclusive(inconclusive) {
  // set the summary and verbose messages
  setmsg(msg);
}

ErrorLogger::ErrorMessage::ErrorMessage(
    const std::list<const Token *> &callstack, const TokenList *list,
    Severity::SeverityType severity, const std::string &id,
    const std::string &msg, bool inconclusive)
    : _id(id), _severity(severity), _cwe(0U), _inconclusive(inconclusive) {
  // Format callstack
  for (std::list<const Token *>::const_iterator it = callstack.begin();
       it != callstack.end(); ++it) {
...
ErrorLogger::ErrorMessage::ErrorMessage(
    const std::list<const Token *> &callstack, const TokenList *list,
    Severity::SeverityType severity, const std::string &id,
    const std::string &msg, const CWE &cwe, bool inconclusive)
...
ErrorLogger::ErrorMessage::ErrorMessage(const ErrorPath &errorPath,
                                        ...
                                        const char id[], const std::string &msg,
                                        const CWE &cwe, bool inconclusive)
    : _id(id), _severity(severity), _cwe(cwe.id), _inconclusive(inconclusive) {
  // Format callstack
  for (ErrorPath::const_iterator it = errorPath.begin(); it != errorPath.end();
       ++it) {
...
ErrorLogger::ErrorMessage::ErrorMessage(
    const tinyxml2::XMLElement *const errmsg)
    : _severity(Severity::none), _cwe(0U), _inconclusive(false) {
  ...
  for (const tinyxml2::XMLElement *e = errmsg->FirstChildElement(); e;
...
void ErrorLogger::ErrorMessage::setmsg(const std::string &msg) {
  ...
  const std::string symbolName =
      mSymbolNames.empty() ? std::string()
                           : mSymbolNames.substr(0, mSymbolNames.find('\n'));
...
Suppressions::ErrorMessage
ErrorLogger::ErrorMessage::toSuppressionsErrorMessage() const {
...
// There is no utf-8 support around but the strings should at least be safe for
// to tinyxml2. See #5300 "Invalid encoding in XML output" and  #6431 "Invalid
// XML created - Invalid encoding of string literal "
std::string ErrorLogger::ErrorMessage::fixInvalidChars(const std::string &raw) {
  ...
  while (from != raw.end()) {
    if (std::isprint(static_cast<unsigned char>(*from))) {
      ...
      es << '\\' << std::setbase(8) << std::setw(3) << std::setfill('0')
         << uFrom;
      result += es.str();
    }
    ++from;
...
std::string ErrorLogger::ErrorMessage::toXML() const {
  ...
  for (std::list<FileLocation>::const_reverse_iterator it = _callStack.rbegin();
       it != _callStack.rend(); ++it) {
...
static std::string readCode(const std::string &file, unsigned int linenr,
                            unsigned int column, const char endl[]) {
...
std::string
ErrorLogger::ErrorMessage::toString(bool verbose,
  ...
  // Substitution should be done first so messages from cppcheck never get
  // translated.
  ...
  if (result.find("{inconclusive:") != std::string::npos) {
    ...
    const std::string replaceWith =
        _inconclusive ? result.substr(pos1 + 14, pos2 - pos1 - 14)
                      : std::string();
  ...
  findAndReplace(result, "{message}",
                 verbose ? mVerboseMessage : mShortMessage);
  findAndReplace(result, "{callstack}",
                 _callStack.empty() ? emptyString
                                    : callStackToString(_callStack));
  if (!_callStack.empty()) {
    findAndReplace(result, "{file}", _callStack.back().getfile());
    findAndReplace(result, "{line}", MathLib::toString(_callStack.back().line));
    findAndReplace(result, "{column}",
                   MathLib::toString(_callStack.back().col));
    if (result.find("{code}") != std::string::npos) {
      ...
      findAndReplace(result, "{code}",
                     readCode(_callStack.back().getOrigFile(),
                              _callStack.back().line, _callStack.back().col,
                              endl));
    }
  } else {
  ...
  if (!templateLocation.empty() && _callStack.size() >= 2U) {
    for (const FileLocation &fileLocation : _callStack) {
      ...
      findAndReplace(text, "{info}",
                     fileLocation.getinfo().empty() ? mShortMessage
                                                    : fileLocation.getinfo());
      if (text.find("{code}") != std::string::npos) {
        ...
        findAndReplace(text, "{code}",
                       readCode(fileLocation.getOrigFile(), fileLocation.line,
                                fileLocation.col, endl));
      }
      result += '\n' + text;
    }
  }
...
bool ErrorLogger::reportUnmatchedSuppressions(
    const std::list<Suppressions::Suppression> &unmatched) {
  bool err = false;
  ...
  for (const Suppressions::Suppression &s : unmatched) {
    ...
    for (const Suppressions::Suppression &s2 : unmatched) {
      if (s2.errorId == "unmatchedSuppression") {
        if ((s2.fileName == "*" || s2.fileName == s.fileName) &&
            (s2.lineNumber == Suppressions::Suppression::NO_LINE ||
             s2.lineNumber == s.lineNumber)) {
          suppressed = true;
          break;
        }
      }
    ...
    reportErr(ErrorLogger::ErrorMessage(
        callStack, emptyString, Severity::information,
        "Unmatched suppression: " + s.errorId, "unmatchedSuppression", false));
    err = true;
  }
  return err;
}
...
std::string ErrorLogger::callStackToString(
  ...
  for (std::list<ErrorLogger::ErrorMessage::FileLocation>::const_iterator tok =
           callStack.begin();
       tok != callStack.end(); ++tok) {
...
ErrorLogger::ErrorMessage::FileLocation::FileLocation(
    const Token *tok, const TokenList *tokenList)
    : fileIndex(tok->fileIndex()), line(tok->linenr()), col(tok->col()),
      mOrigFileName(tokenList->getOrigFile(tok)),
      mFileName(tokenList->file(tok)) {}

ErrorLogger::ErrorMessage::FileLocation::FileLocation(
    const Token *tok, const std::string &info, const TokenList *tokenList)
    : fileIndex(tok->fileIndex()), line(tok->linenr()), col(tok->col()),
      mOrigFileName(tokenList->getOrigFile(tok)),
      mFileName(tokenList->file(tok)), mInfo(info) {}

std::string
ErrorLogger::ErrorMessage::FileLocation::getfile(bool convert) const {
  if (convert)
...
std::string
ErrorLogger::ErrorMessage::FileLocation::getOrigFile(bool convert) const {
  if (convert)
...
std::string replaceStr(std::string s, const std::string &from,
                       const std::string &to) {
